...
def talk_grabber(reader):
  ...
  last_time = datetime.time(*map(int, first_row[2].split(":")))
  end_time = datetime.time(*map(int, first_row[3].split(":")))
  start_dt = datetime.datetime.combine(this_date, last_time)
  end_dt = datetime.datetime.combine(this_date, end_time)
  authors = ".".join(first_row[6].split(".")[:-1])
  title = first_row[6].split(".")[-1].strip()
  talks.append(Talk(start_dt, end_dt, title, authors))

  for row in reader:
    start_time = datetime.time(*map(int, row[2].split(":")))
    if (start_time < last_time):
      this_date = datetime.date(2012, 7, this_date.day + 1)
    last_time = start_time
    end_time = datetime.time(*map(int, row[3].split(":")))
    start_dt = datetime.datetime.combine(this_date, last_time)
    end_dt = datetime.datetime.combine(this_date, end_time)
    authors = ".".join(row[6].split(".")[:-1])
    title = row[6].split(".")[-1].strip()
    talks.append(Talk(start_dt, end_dt, title, authors))
